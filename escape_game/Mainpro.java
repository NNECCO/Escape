...
public class Mainpro extends Applet implements KeyListener,MouseListener,MouseMotionListener,Runnable {
	...
	public void paint(Graphics g) {
		...
		if (is_show_buttons) {
			if      (is_show_bag)    show_bag();
			else if (is_show_hint)   show_hint();
			else if (is_examine)     examine("");
			else if (is_show_trophy) show_trophy();
		}
		...
		is_next = false;
		if (screen_error == true) show_screen_error();
	...
	void show_current_time() {
		...
		buffer.drawString("("+player_x+","+player_y+")", 400, 400);
	}
		...
		if (bag.size() != 0) {
			for (int i = (show_bag_page-1)*5;i < (show_bag_page-1)*5+5;i++) {
				if (i >= bag.size()) { //bag内のアイテム数がアイテムナンバー(ページ数*5+i)以下-> (1-1)*5+i>= アイテム数0
					...
					int item_index = i;
					buffer.drawImage(name_to_icon.get(bag.get(item_index))
							, sb_left+1, sb_top+item_icon_size*(i%5) + 1 
							, item_icon_size-1, item_icon_size-1, this);
					buffer.drawString(bag.get(item_index)
						, sb_left + item_icon_size + 5
						, (sb_top + separate_size * (i%5) + 15));
					//使うボタン
					button_use_item[i%5].setLocation(sb_left+sb_width-button_use_item[i%5].getWidth(), sb_top + separate_size * (i%5) + 25);
				}
			}
		}
		else {
			buffer.drawString("nothing in the bag"
					, sb_left + item_icon_size + 10
					, sb_top + (separate_size / 10) * 6);
		}
	...
	void examine(String item) {//pressed examine button
		...
		if (character_direct.equals("back")) {
			for (int i = 1;i <= examine_range;i++) {
				...
				if (here_now.equals(examine_point));//今いる地点と同じなら何もしない
	...
	void show_trophy() {//pressed show-trophy button
		...
		buffer.drawLine(st_left+item_icon_size, st_top+20, st_left+item_icon_size, st_top+item_icon_size*4+20);//仕切り縦線
		for (int i = 0;i < 5;i++) {
			buffer.drawLine(st_left, st_top + item_icon_size*i+20, st_left+st_width, st_top + item_icon_size*i+20);//仕切り横線
		}
		buffer.setFont(mp15);
	...
	ArrayList<String> message_separate(String statement, int s_size) { 
		...
		for (int i = 0;i < char_array.length;i++) {
			...
			int char_size = 0;
			if (a.getBytes().length == 1) char_size = 1;
			else                          char_size = 2;
	...
	public void show_map() {
		if (screenMode.equals("WesternStyleRoom")) {
			now_field.showMap(this);
		} else if(screenMode.equals("Dk")) {
			now_field.showMap(this);
		} else if(screenMode.equals("DkTop")) {
			now_field.showMap(this);
		} else if(screenMode.equals("Entrance")) {
			now_field.showMap(this);
		} else if(screenMode.equals("JapaneseStyleRoom1")) {
			now_field.showMap(this);
		} else if(screenMode.equals("Datuijo")) {
			now_field.showMap(this);
		}
	}
	
	public void show_me() {
		if (character_direct.equals("front"))      buffer.drawImage(c_f, player_x, player_y, character_size_x, character_size_y,this);
		else if (character_direct.equals("left"))  buffer.drawImage(c_l, player_x, player_y, character_size_x, character_size_y,this);
		else if (character_direct.equals("back"))  buffer.drawImage(c_b, player_x, player_y, character_size_x, character_size_y,this);
		else if (character_direct.equals("right")) buffer.drawImage(c_r, player_x, player_y, character_size_x, character_size_y,this);
	}
						...
						if (character_direct.equals("back")) {
							if (now_field.is_character_in(player_x, player_y+vy)) {
								...
								if(10 <= player_x && player_x <= 410 && 380 <= player_y && now_field == dkTop) {
									/* DkTop -> Dk */
									now_field = dk; screenMode = "Dk"; now_field.setImages(getCodeBase());
									player_y = -10;
								} else if(150 <= player_x && player_x <= 310 && 340 <= player_y && now_field == entrance) {
									/* Entrance -> DkTop */
									now_field = dkTop; screenMode = "DkTop"; now_field.setImages(getCodeBase());
									player_x = 190; player_y = 80;
									
								} else if(170 <= player_x && player_x <= 230 && 310 <= player_y && now_field == dk) {
									/* Dk -> westroom */
									now_field = westernStyleRoom; screenMode = "WesternStyleRoom"; now_field.setImages(getCodeBase());
									player_x= 40; player_y = 30;
								} else if(10 <= player_x && player_x <= 70 && 310 <= player_y && now_field == dk && dk.isDoor_wasitu1()) {
									/* Dk -> JapaneseStyleRoom1 */
									now_field = japaneseStyleRoom1; screenMode = "JapaneseStyleRoom1"; now_field.setImages(getCodeBase());
									player_x= 300; player_y = 30;
								}
							}
						}
						else {
							if (character_direct.equals("left")
								...
								if (now_field.is_character_in(px,py)) {
									...
									if(character_direct.equals("right")) {
										if (now_field.is_character_in(player_x-10,player_y)) player_x -= 10;
										else message_add("あぁ...、後ろもさがれねぇわ...");
									}
									if(character_direct.equals("left"))  {
										if (now_field.is_character_in(player_x+10,player_y)) player_x += 10;
										else message_add("あぁ...、後ろもさがれねぇわ...");
							...
							else if (character_direct.equals("front")) character_direct = "back";
						}
					}
				}
				break;
						...
						if (character_direct.equals("left")) {
							if (now_field.is_character_in(player_x-vx, player_y)) {
								...
								if(player_x < 120 && 180 <= player_y && player_y <= 220 && now_field == dkTop) {
									/* DkTop -> Datuijo */
									now_field = datuijo; screenMode = "Datuijo"; now_field.setImages(getCodeBase());
									player_x = 330; player_y = 260;
								}
							}
						}
						else {
							if (character_direct.equals("front")
								...
								if (now_field.is_character_in(px,py)) {
									...
									message_add("おっと、少し下がらないと周りに当たってまう..");
									if(character_direct.equals("back")) {
										if (now_field.is_character_in(player_x,player_y-10)) player_y -= 10;
										else message_add("あぁ...、後ろもさがれねぇわ...");
									}
									if(character_direct.equals("front"))  {
										if (now_field.is_character_in(player_x,player_y+10)) player_y += 10;
							...
							else if (character_direct.equals("right")) character_direct = "left";
						}
					}
				}
				break;
					...
					if (screenMode.equals(now_field.toString())) {
						if (character_direct.equals("front")) {
							if (now_field.is_character_in(player_x, player_y-vy)) {
								 ...
								 * 部屋移動の処理
								 */
								if(10 <= player_x && player_x <= 80 && player_y <= 0 && westernStyleRoom.get_is_open_door() == true && now_field == westernStyleRoom) {
									/* westroom -> Dk */
									now_field = dk; screenMode = "Dk"; now_field.setImages(getCodeBase());
									player_x = 190; player_y = 300;
									...
									/* DkTop -> Entrance */
									now_field = entrance; screenMode = "Entrance"; now_field.setImages(getCodeBase());
									player_y = 300;
								} else if(280 <= player_x && player_x <= 320 && player_y <= 20 && now_field == japaneseStyleRoom1) {
									// JSRoom1 -> Dk
									now_field = dk; screenMode = "Dk"; now_field.setImages(getCodeBase());
									player_x = 40; player_y = 300;
								}
							}
						}
						else {
							...
							else if (character_direct.equals("back")) character_direct = "front";
						}
					}
				}
				break;
				...
				if (is_show_character == true) {
					if(screenMode.equals(now_field.toString())) {
						if (character_direct.equals("right")) {
							...
							else if (character_direct.equals("left")) character_direct = "right";
						}
					}
				}
				break;
		...
		try {
			...
			while(data != null) {
				if(data.substring(0, 2).equals("//")) {	//コメント行
					...
					if (dataLabels != null) dataLabel_woLP = dataLabels[0];
					if (dataLabel.equals("playerX")) {
						...
						if (fieldName.equals("Common")) { //共通のフラグ
							...
							for (int index = 0; index < flags.length; index++) {
								if (flags[index].equals("nothing")) break;
	...
	Field getField(String fieldName) {
		if (fieldName.equals("WesternStyleRoom")) return westernStyleRoom;
		else if (fieldName.equals("Dk")) return dk;
		else if (fieldName.equals("DkTop")) return dkTop;
		else if (fieldName.equals("Entrance")) return entrance;
		else if (fieldName.equals("JapaneseStyleRoom1")) return japaneseStyleRoom1;
		else if (fieldName.equals("Datuijo")) return datuijo;
		else return null;
	}
	...
	void setFlagTrue(String flagString) {
		if (flagString.equals("is_already_checked_bag")) is_already_checked_bag = true;
